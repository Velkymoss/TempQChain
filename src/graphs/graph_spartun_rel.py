...
with Graph("spatial_QA_rule") as graph:
    ...
    for rel1, rel2 in transitive_2:
        ifL(
            andL(rel2("x"), existsL(transitive("t", path=("x", transitive))), rel1(path=("t", tran_quest2))),
    ...
    for rel1 in transitive_3_1:
        for rel2 in transitive_3_2:
            ifL(
                andL(rel1("x"), existsL(transitive("t", path=("x", transitive))), rel2(path=("t", tran_quest2))),
                rel2(path=("t", tran_quest3)),
            )

    # Transitive + topo constrains
    tran_topo = Concept(name="transitive_topo")
    tran_topo_quest1, tran_topo_quest2, tran_topo_quest3, tran_topo_quest4 = transitive.has_a(
        arg111=question, arg222=question, arg333=question, arg444=question
    )
