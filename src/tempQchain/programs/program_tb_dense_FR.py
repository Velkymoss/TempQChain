...
def program_declaration_tb_dense_fr_T5(
    ...
    question[story_contain, "question", "story", "relation", "id", "text_labels"] = JointSensor(
        story["questions"],
        story["stories"],
        story["relations"],
        story["question_ids"],
    ...
    question[output_for_loss] = JointSensor(story_contain, "question", "story", forward=t5_inTokenizer, device=device)

    question["input_ids"] = JointSensor(story_contain, "question", "story", True, forward=t5_inTokenizer, device=device)

    question[output_for_loss] = FunctionalSensor(
    ...
    def read_decoder(_, decoder_list):
        text_label = [[0] * LABEL_DIM for _ in range(len(decoder_list))]
    ...
    question["output_relations"] = FunctionalSensor(
        story_contain, "output_decoder", forward=read_decoder, device=device
    )

    question[after] = FunctionalSensor(story_contain, "output_relations", 0, forward=read_label, device=device)
    question[before] = FunctionalSensor(story_contain, "output_relations", 1, forward=read_label, device=device)
    question[includes] = FunctionalSensor(story_contain, "output_relations", 2, forward=read_label, device=device)
    question[is_included] = FunctionalSensor(story_contain, "output_relations", 3, forward=read_label, device=device)
    question[simultaneous] = FunctionalSensor(story_contain, "output_relations", 4, forward=read_label, device=device)
    question[vague] = FunctionalSensor(story_contain, "output_relations", 5, forward=read_label, device=device)
...
def program_declaration_tb_dense_fr_T5_v2(
    ...
    def read_decoder(_, decoder_list):
        text_label = [[0] * 6 for _ in range(len(decoder_list))]  # Changed from 15 to 6
        for ind, text_decode in enumerate(decoder_list):
            all_relations = text_decode.strip()
            for label in all_labels:
                if all_relations.find(label + ":" + "yes") != -1:  # Fixed the condition
...
def program_declaration_tb_dense_fr_T5_v3(
    ...
    def make_question(questions, stories, relations, q_ids, labels):
        all_labels_data = make_labels(labels)
    ...
    question["label_input_ids"] = JointSensor(story_contain, "text_label", True, forward=t5_outtokenizer, device=device)

    question["hidden_layer"] = ModuleLearner(
...
# def program_declaration_spartun_fr_T5_v4(
#     device, *, pmd=False, beta=0.5, sampling=False, sampleSize=1, dropout=False, constraints=False, spartun=True
# ):
#     program = None
...
#     t5_model = T5WithLoraGenerativeCLF2(
#         "google/flan-t5-base",
...
#             all_labels_group = ["none"] * group_number
#             for label in labels:
#                 label_group = group_label[label]
#                 if all_labels_group[label_group] != "none":
#                     print("ERROR")
#                 all_labels_group[label_group] = label

#             all_labels_group = [
#                 t5_normal_tokenizer.decode(token_map[label], skip_special_tokens=True) for label in all_labels_group
#             ]
...
#     question["generate_answer"] = FunctionalSensor(story_contain, "input_ids", forward=t5_model.generate, device=device)
...
#     candidate_output = ["left", "right", "none"]
#     index_labels = {"left": 0, "right": 1}
#     candidate_output_token = transform_label_token(candidate_output, token_map_normalize)

#     first_token_clf = T5LocationClassification(
#         token_loc=[0, token_each_label], candidate_output_token=candidate_output_token, device=device
#     )
#     question["first_token_prob"] = ModuleLearner(story_contain, "hidden_layer", module=first_token_clf)

#     question[left] = ModuleLearner(
#         story_contain, "first_token_prob", module=LabelClassification(index_label=index_labels["left"]), device=device
#     )
...
#         story_contain, "first_token_prob", module=LabelClassification(index_label=index_labels["right"]), device=device
#     )
#     question[right] = FunctionalSensor(story_contain, "right_label", forward=read_label, label=True, device=device)
#     # 2. Above - Below
#     candidate_output = ["above", "below", "none"]
#     index_labels = {"above": 0, "below": 1}
#     candidate_output_token = transform_label_token(candidate_output, token_map_normalize)

#     second_token_clf = T5LocationClassification(
#         token_loc=[token_each_label, token_each_label * 2], candidate_output_token=candidate_output_token, device=device
#     )
#     question["second_token_prob"] = ModuleLearner(story_contain, "hidden_layer", module=second_token_clf)

#     question[above] = ModuleLearner(
#         story_contain, "second_token_prob", module=LabelClassification(index_label=index_labels["above"]), device=device
#     )
...
#         story_contain, "second_token_prob", module=LabelClassification(index_label=index_labels["below"]), device=device
#     )
#     question[below] = FunctionalSensor(story_contain, "below_label", forward=read_label, label=True, device=device)

#     # 3.Behind - Front
#     candidate_output = ["behind", "front", "none"]
#     index_labels = {"behind": 0, "front": 1}
#     candidate_output_token = transform_label_token(candidate_output, token_map_normalize)

#     third_token_clf = T5LocationClassification(
#         token_loc=[token_each_label * 2, token_each_label * 3],
#         candidate_output_token=candidate_output_token,
#         device=device,
#     )
#     question["third_token_prob"] = ModuleLearner(story_contain, "hidden_layer", module=third_token_clf)
#     question[behind] = ModuleLearner(
#         story_contain, "third_token_prob", module=LabelClassification(index_label=index_labels["behind"]), device=device
#     )
...
#         story_contain, "third_token_prob", module=LabelClassification(index_label=index_labels["front"]), device=device
#     )
...
#     candidate_output = ["disconnected", "touch", "overlap", "none"]
#     index_labels = {"disconnected": 0, "touch": 1, "overlap": 2}
#     candidate_output_token = transform_label_token(candidate_output, token_map_normalize)

#     forth_token_clf = T5LocationClassification(
#         token_loc=[token_each_label * 3, token_each_label * 4],
#         candidate_output_token=candidate_output_token,
#         device=device,
#     )
#     question["forth_token_prob"] = ModuleLearner(story_contain, "hidden_layer", module=forth_token_clf)
...
#         story_contain, "forth_token_prob", module=LabelClassification(index_label=index_labels["touch"]), device=device
#     )
...
#     # 5. Near - Far
#     candidate_output = ["near", "far", "none"]
#     index_labels = {"near": 0, "far": 1}
#     candidate_output_token = transform_label_token(candidate_output, token_map_normalize)

#     fifth_token_clf = T5LocationClassification(
#         token_loc=[token_each_label * 4, token_each_label * 5],
#         candidate_output_token=candidate_output_token,
#         device=device,
#     )
#     question["fifth_token_prob"] = ModuleLearner(story_contain, "hidden_layer", module=fifth_token_clf)
#     question[near] = ModuleLearner(
#         story_contain, "fifth_token_prob", module=LabelClassification(index_label=index_labels["near"]), device=device
#     )
...
#         story_contain, "fifth_token_prob", module=LabelClassification(index_label=index_labels["far"]), device=device
#     )
...
#     candidate_output = ["covered by", "inside", "cover", "contain", "none"]
#     index_labels = {"coveredby": 0, "inside": 1, "cover": 2, "contain": 3}
#     candidate_output_token = transform_label_token(candidate_output, token_map_normalize)

#     sixth_token_clf = T5LocationClassification(
#         token_loc=[token_each_label * 5, token_each_label * 6],
#         candidate_output_token=candidate_output_token,
#         device=device,
#     )
#     question["sixth_token_prob"] = ModuleLearner(story_contain, "hidden_layer", module=sixth_token_clf)
...
#         story_contain, "sixth_token_prob", module=LabelClassification(index_label=index_labels["inside"]), device=device
#     )
...
#         story_contain, "sixth_token_prob", module=LabelClassification(index_label=index_labels["cover"]), device=device
#     )
...
# # Answer in the form of left:exist, right:not exist, above:not exist, above:exist
# def program_declaration_spartun_fr_T5_v5(
#     device, *, pmd=False, beta=0.5, sampling=False, sampleSize=1, dropout=False, constraints=False, spartun=True
# ):
#     program = None
#     from graphs.graph_spartun_rel import (
...
#     t5_model = T5WithLoraGenerativeCLF3(
#         "google/flan-t5-base", labels=all_labels_text, tokenizer=t5_normal_tokenizer, device=device, adapter=True
#     )
...
#                 text_label[ind] += all_labels_text[i] + ":"
#                 text_label[ind] += "exist" if bits_label & label_bit[label] else "not exists"
#                 text_label[ind] += ","
#         # label_nums = [0 if label == "Yes" else 1 if label == "No" else 2 for label in labels]
#         return text_label
...
#         contain,
#     ]
#     for i, text_label in enumerate(all_labels_text):
#         concept = labels_concept[i]
#         question[concept] = ModuleLearner(
#             story_contain,
#             "hidden_layer",
#             module=T5LabelLocationClassification(
#                 token_loc=[token_each_label * i, token_each_label * (i + 1)], label=text_label, map_label_loc=token_map
#             ),
#             device=device,
#         )

#         question[concept] = FunctionalSensor(
#             story_contain, all_labels[i] + "_label", forward=read_label, label=True, device=device
#         )
#     # question[left] = ModuleLearner(story_contain, "hidden_layer",
#     #                                module=
#     #                                T5LabelLocationClassification(
#     #                                    token_loc=[token_each_label * 0, token_each_label * 1],
#     #                                    label="left", map_label_loc=token_map),
#     #                                device=device)
#     #
#     # question[left] = FunctionalSensor(story_contain, "left_label", forward=read_label, label=True, device=device)
#     #
...
#     #                                module=
#     #                                T5LabelLocationClassification(
#     #                                    token_loc=[token_each_label * 1, token_each_label * 2],
#     #                                    label="right", map_label_loc=token_map),
#     #                                device=device)
#     # question[right] = FunctionalSensor(story_contain, "right_label", forward=read_label, label=True, device=device)
#     #
#     # question[left] = ModuleLearner(story_contain, "hidden_layer",
...
#     #                                T5LabelLocationClassification(
#     #                                    token_loc=[token_each_label * 2, token_each_label * 3],
#     #                                    label="above", map_label_loc=token_map),
#     #                                device=device)
#     # question[above] = FunctionalSensor(story_contain, "above_label", forward=read_label, label=True, device=device)
#     #
#     # question[below] = ModuleLearner(story_contain, "hidden_layer",
#     #                                 module=
#     #                                 T5LabelLocationClassification(
#     #                                     token_loc=[token_each_label * 3, token_each_label * 4],
#     #                                     label="below", map_label_loc=token_map),
#     #                                 device=device)
#     #
#     # question[below] = FunctionalSensor(story_contain, "below_label", forward=read_label, label=True, device=device)
#     # question[behind] = FunctionalSensor(story_contain, "behind_label", forward=read_label, label=True, device=device)
