...
def program_declaration_spartun_fr(device, *, pmd=False, beta=0.5, sampling=False, sampleSize=1, dropout=False,
    ...
    program = None
    from domino_graphs.graph_spartun_rel import graph, story, story_contain, question, \
...
def program_declaration_spartun_fr_T5_v4(device, *, pmd=False, beta=0.5, sampling=False, sampleSize=1, dropout=False,
    ...
    def make_text_labels(label_list):
        ...
        for ind, bits_label in enumerate(labels):
            ...
            all_labels_group = [t5_normal_tokenizer.decode(token_map[label], skip_special_tokens=True) for label in
                                all_labels_group]
    ...
    question["generate_answer"] = FunctionalSensor(story_contain, "input_ids",
                                                   forward=t5_model.generate, device=device)
...
def program_declaration_spartun_fr_T5_v5(device, *, pmd=False, beta=0.5, sampling=False, sampleSize=1, dropout=False,
                                         constraints=False, spartun=True):
    program = None
    from domino_graphs.graph_spartun_rel import graph, story, story_contain, question, \
        left, right, above, below, behind, front, near, far, disconnected, touch, \
    ...
    group_label = {"left": 0, "right": 1,
                   "above": 2, "below": 3,
                   "behind": 4, "front": 5,
                   "overlap": 6, "disconnected": 7, "touch": 8,
                   "near": 9, "far": 10,
                   "covered by": 11, "inside": 12, "cover": 13, "contain": 14}
    ...
    t5_model = T5WithLoraGenerativeCLF3("google/flan-t5-base",
                                        labels=all_labels_text,
                                        tokenizer=t5_normal_tokenizer,
                                        device=device,
                                        adapter=True)
    ...
    # Empty prediction (both classes are 0)
    all_labels_text = ["left", "right", "above", "below", "overlap", "behind", "front",
                       "near", "far", "disconnected", "touch", "covered by",
                       "inside", "cover", "contain"]
    all_labels = ["left", "right", "above", "below", "po", "behind", "front", "near", "far", "dc", "ec", "tpp", "ntpp",
                  "tppi", "ntppi"]
    labels_concept = [left, right, above, below, overlap, behind, front,
                      near, far, disconnected, touch, coveredby,
                      inside, cover, contain]
    for i, text_label in enumerate(all_labels_text):
        concept = labels_concept[i]
        question[concept] = ModuleLearner(story_contain, "hidden_layer",
                                          module=
                                          T5LabelLocationClassification(
                                              token_loc=[token_each_label * i, token_each_label * (i + 1)],
                                              label=text_label, map_label_loc=token_map),
                                          device=device)

        question[concept] = FunctionalSensor(story_contain, all_labels[i]+"_label",
                                             forward=read_label, label=True, device=device)
